...
public class MaquinaL {
    ...
    JdbcTemplate database2 = connection2.getConnectionMySql();

//    public MaquinaL getMaquina2(Integer idMaquina) {
//        List<MaquinaL> maquinas = database2.query("select * from maquinas where idMaquina = ?", new BeanPropertyRowMapper<>(MaquinaL.class), idMaquina);
//        MaquinaL maquinal = maquinas.get(maquinas.size());
//        return maquinal;
//    }
    ...
    public void setarInfos(Integer idMaquina) {
        ...
        database2.update("UPDATE maquinas SET "
                + "disco = ?"
                + ",memoria = ?"
                + ",processador = ?"
                + ",quantidadeDiscos = ?"
